...
### Target repo cache & subdir URLs (`DockerManager.clone_repo`)
`repo_url` may be a plain repo URL **or** a browser `tree`/`blob` URL pointing at a subdirectory — e.g. `https://github.com/vulhub/vulhub/tree/master/node/CVE-2017-14849`. `_parse_repo_url` splits it into the repo to clone (`…/vulhub/vulhub.git`), the ref (`master`), and the subdir (`node/CVE-2017-14849`); GitLab's `/-/tree/` form works too. The **whole repo** is cloned **once** into a persistent cache at `settings.local_repos_dir` (`VULBOX_LOCAL_REPOS_DIR`, default `/home/code/local-repos`) under a stable per-repo slug (`github.com__vulhub__vulhub`) — so every vulhub app shares one clone — and `clone_repo` returns the **subdir path**, which is what the pipeline builds, stack-detects, and reads `.vulbox.yml` from. On a re-run the cached checkout is reused: VulBox attempts a shallow `git fetch` + `reset --hard FETCH_HEAD`, but a **failed update degrades to the cached copy** instead of failing the build — so a transient remote pull error no longer blocks an assessment once the repo has been cloned at least once. Only a first-ever clone (no cache) can raise `BuildFailedError`. `file://` and bare local paths are referenced in place (no clone) — that's the path the e2e/vulhub harnesses use. Caveat: the ref is taken as the single segment after `tree`/`blob`, so branch names containing `/` aren't supported (fine for `master`/`main`/tags).

**Sandbox read-only posture.** A target with no `.vulbox.yml` gets `_default_sandbox_config()`, whose `read_only` is resolved from `VULBOX_SANDBOX_READ_ONLY` (default **false**) at call time. False is the default because most real server stacks (tomcat/apache/rails/elasticsearch) write logs/work/data dirs on boot and crash under `--read-only`, failing the run before any scan/test — so a paste-the-URL demo works out of the box. Network still defaults to `none`, and a target's `.vulbox.yml` `sandbox.read_only` still overrides per-target. Set `VULBOX_SANDBOX_READ_ONLY=true` to re-lock the global default.
...
- **Falco detection rules** (`deploy/falco/vulbox_rules.yaml`): the detectability dimension depends on Falco actually firing on ART activity. The stock ruleset did **not** — Falco launched and ran but wrote no `falco.json` (its `file_output` only creates the file once a rule matches), because the container engine didn't attach container metadata to the docker-exec'd ART syscalls, so container-scoped stock rules never tripped. VulBox ships its own self-contained rules tuned to the exact syscalls `atomic_runner.sh` produces (sensitive-file read, `/proc/1/root` host access, systemd/library writes, pkexec, shell, discovery), each MITRE-tagged. The high-signal rules are **syscall-scoped, not container-scoped**, so they fire whether or not container metadata attaches. `FalcoAdapter.attach` loads them via `-r` alongside auto-detected stock rules (`_rules_args`), and the `--profile full` compose sidecar mounts the file into `/etc/falco/rules.d/`. Verify on a Falco host with `scripts/falco_smoke.sh`. The post-launch settle (`_FALCO_STARTUP_SETTLE_SECS`, 6s) also covers BPF driver load so the first test isn't fired before Falco is capturing.
...
- `LLMRemediationService` (`app/services/llm_remediation.py`) generates one remediation per SecurityMatrixEntry; entries at/above `VULBOX_LLM_MIN_RISK_SCORE` (default **25** = the report's high/orange band, so high + critical findings qualify) get a Gemini call, the rest use the static `RemediationService` rule. Two quota guards keep free-tier usage bounded: the risk threshold, and a hard per-run ceiling `VULBOX_LLM_MAX_CALLS` (default **15**, `≤0` = unlimited). Entries are processed highest-risk-first, so the top-N qualifying findings get the LLM and the long tail degrades to static once either guard trips. `RunSummaryService` adds one run-level executive summary.
...
- Env vars: `VULBOX_LLM_REMEDIATION` (enable, default false), `GEMINI_API_KEY`, `VULBOX_LLM_MODEL_PRIMARY`, `VULBOX_LLM_MODEL_BACKUP`, `VULBOX_LLM_MIN_RISK_SCORE`, `VULBOX_LLM_MAX_CALLS`, `VULBOX_LLM_TIMEOUT_SECS`, `VULBOX_LLM_MAX_TOKENS`, `VULBOX_LLM_EXEC_SUMMARY` (default true).
